...
function App() {
  ...
  function AlertExtreme(weather) {
    console.log(weather);

    if (weather === "Snow" || weather === "Thunderstorm" || weather === "Mist") {
      return <AlertDismissible weather={weather} variant={true} />;
    }
  }

  function AlertDismissible({ weather = "default", variant = false }) {
    const [show, setShow] = useState(variant);

    if (show) {
      return (
        <Alert variant="danger" onClose={() => setShow(false)} dismissible>
          <Alert.Heading>Oh snap! Extreme Weather Warning!</Alert.Heading>
          <p>
            The Following Weather is {weather}, please be careful!
          </p>
        </Alert>
      );
    }
  ...
  return (
    <div className="app">
      ...
      {(typeof forecast.list !== "undefined") ? (
        <div>
          {AlertExtreme(forecast.list[0].weather[0].main)}
